...
function activate(context) {
    const compileCommand = vscode.commands.registerCommand("moonscript.compile", () => {
        ...
        (0, child_process_1.exec)(`moonc "${filePath}" -o "${outFile}"`, (error, stdout, stderr) => {
            if (error) {
                vscode.window.showErrorMessage(stderr || error.message);
                return;
            }
            vscode.window.showInformationMessage(`Compiled MoonScript → ${path.basename(outFile)}`);
        });
    });
